...
export default function Season() {
  ...
  const seed = useMemo(() => (
    seasonalRecommendations[term?.term_name] || seasonalRecommendations.default
  ), [term?.term_name]);
      ...
      <div className="consumer-page-inner">
        <header className="consumer-page-heading">
          <div>
            ...
            <h1>節氣與當季推薦</h1>
            <p>用白話了解現在的節氣，挑選適合當季的食材。</p>
          </div>
        </header>

        {loading && <LoadingState label="正在讀取目前節氣…" />}

        {error && (
          <EmptyState
            title="節氣資料暫時無法取得"
            description="正式節氣 API 沒有回應，請稍後再試。"
            action={(
              <button type="button" className="consumer-link" onClick={loadTerm}>
        ...
        {!loading && !error && (
          <>
            <section className="season-hero">
              ...
              <div>
                <span>現在的節氣</span>
                <h2>{term?.term_name || seed.solarTerm}</h2>
                <p>
                  {seasonName} · 下一個節氣：{term?.next_term_name || '資料未提供'}
            ...
            <div className="season-grid">
              <Card>
                <Leaf className="season-card-icon" aria-hidden="true" />
                <h2>適合買什麼</h2>
                <p>這些是一般節氣知識推薦，實際價格請先查看今天菜價。</p>
                <div className="season-tags">
                  {seed.recommendedProducts.map((item) => (
                    <button
                      type="button"
                      key={item}
              ...
              <Card>
                <CloudSun className="season-card-icon" aria-hidden="true" />
                <h2>天氣影響提醒</h2>
                <p>
                  {seed.weatherRiskProducts.join('、')}可能受季節天氣影響；
                  這裡是一般風險知識，尚未接入即時天氣 API。
                </p>
              </Card>

              <Card>
                <Soup className="season-card-icon" aria-hidden="true" />
                <h2>節氣料理建議</h2>
                {seed.cookingSuggestions.map((item) => <p key={item}>{item}</p>)}
              </Card>

              <Card>
                <h2>節氣小知識</h2>
                <p>{seed.knowledge}</p>
                <small>{seed.sourceNote}</small>
              </Card>
            </div>

            <button
              type="button"
              className="season-price-cta"
              onClick={() => navigate('/search')}
            >
              查看今天菜價 <ArrowRight size={18} />
            </button>
          </>
        )}
      </div>
