...
import { loadBasketProductDetails } from '../lib/basketProducts';
...
function SavedProductsList({ savedProducts, onRemove }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    loadBasketProductDetails(savedProducts)
      .then((next) => {
  ...
  return (
    <div className="mb-grid">
      {savedProducts.map(({ name, market }) => {
        ...
        const unavailable = detail?.dataState === 'unavailable';
        const status = unavailable ? '暫時無法載入' : getPriceStatus(detail);
        ...
        const detailPath = detailMarket
          ? `/product/${encodeURIComponent(name)}?market=${encodeURIComponent(detailMarket)}`
          : `/search?q=${encodeURIComponent(name)}`;
        const detailLabel = detailMarket ? `查看 ${name} 品項詳情` : `前往查價搜尋 ${name}`;
        return (
          <div key={name} className="card basket-product-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span className={`badge ${unavailable ? 'basket-badge-unavailable' : 'badge-green'}`}>{status}</span>
              <button
            ...
            <p className="basket-price">
              {unavailable
                ? '行情暫時無法載入'
                : detail?.today_price == null ? '資料不足' : `${detail.today_price} 元`}
              {' '}<small>{detailMarket || (unavailable ? '請重新載入' : '市場資料未提供')}</small>
            </p>
            {unavailable ? (
              <div className="basket-unavailable" role="alert">
                <p><strong>收藏仍保留</strong>：行情服務暫時未回應，這不代表品項沒有資料。</p>
            ...
            <small className="basket-updated">
              更新：{unavailable ? '等待行情服務回應' : detail?.price_detail?.trans_date || '資料日期未提供'}
            </small>
            <Link
              to={detailPath}
            ...
            >
              <span style={{
                ...
                {detailMarket ? '查看品項詳情 ↗' : '前往查價 ↗'}
              </span>
              <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                {detailMarket ? '查看詳情 →' : '搜尋品項 →'}
              </span>
            </Link>
          </div>
        );
