...
export default function PointsCenter() {
  ...
  return (
    <div className="points-page">
      ...
      <section id="available-coupons" className="points-section"><div className="points-section-title"><div><h2><TicketPercent size={20} />可兌換優惠券</h2><p>每張券只能兌換一次，實際使用規則依商家公告。</p></div></div>
      ...
      <section id="owned-coupons" className="points-section"><div className="points-section-title"><div><h2>我的優惠券</h2><p>兌換成功後，請在結帳或使用優惠時出示兌換碼。</p></div></div>{mine.length === 0 ? <div className="points-empty">還沒有優惠券，先去兌換一張吧。</div> : <div className="owned-coupon-list">{mine.map(coupon => <div className="owned-coupon" key={coupon.id}><div><strong>{coupon.title}</strong><span>{coupon.description || 'SmartBuy AI 優惠券'}</span></div><code>{coupon.redemption_code}</code><small>{formatDate(coupon.created_at)}</small></div>)}</div>}</section>

      <section id="point-history" className="points-section points-history"><div className="points-section-title"><div><h2>點數紀錄</h2><p>每筆獎勵與兌換都會留下紀錄。</p></div></div>{(points?.transactions || []).length === 0 ? <div className="points-empty">目前還沒有點數紀錄。</div> : <div>{points.transactions.map(item => <div className="points-transaction" key={item.id}><div><strong>{item.reason}</strong><small>{formatDate(item.created_at)}</small></div><b className={item.amount > 0 ? 'positive' : 'negative'}>{item.amount > 0 ? '+' : ''}{item.amount} 點</b></div>)}</div>}</section>
    </div>
  );
}
