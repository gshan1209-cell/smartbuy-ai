...
> 本節為事後補充，說明本規格書與目前實際程式碼的落差，其餘章節維持原始 MVP 規劃內容，僅供歷史參考。

以下三項功能已於 2026-07-21 從程式碼中移除：

- **產地天氣風險判斷**（原規格 4.1 F02、5.1 首頁/搜尋菜價頁、6.2/6.3 資料來源、7.2 核心邏輯、14.2 API）
- **颱風 / 天災警示**（原規格 4.1 F03、7.5 採買建議整合邏輯）
- **替代品推薦**（原規格 4.1 F06、7.5、14.4 API）

移除原因：前端（`Home.jsx`、`PriceSearch.jsx`、`ProductDetail.jsx`）從未實際渲染這些功能對應的欄位（`weather_risk`、`weather_detail`、`weather_impact`、`alternatives`），後端算完全被前端丟棄，功能形同虛設，因此整條程式碼鏈（含假資料 CSV、天氣讀取層、風險判斷模組、替代品推薦模組）已一併清除。

現況：

- 採買建議只依「菜價狀態」判斷（偏貴 / 正常 / 便宜 / 資料不足），不再包含天氣風險與替代品建議
...
另外，**24 節氣功能只完成 F11（首頁節氣提醒卡）的簡化版，F12（節氣推薦頁）與 F13（節氣×菜價推薦）從未實作**：

- 現況：後端 `GET /api/solar-term`、`GET /api/home` 會回傳目前節氣（以天文演算法即時計算，非讀取 `data/calendar/solar_terms.csv`），前端 `PriceSearch.jsx` 顯示「現在節氣」提示條；除此之外沒有節氣推薦頁面、沒有節氣品項推薦邏輯，`data/calendar/seasonal_products.csv`、`src/recommendation/seasonal_recommender.py` 已移除，`data/calendar/solar_terms.csv` 為未被讀取的死資料
- 本文件第 4.1 F12/F13、5.1 節氣推薦頁、6.4/6.5、7.3、14.3（部分）、15.2（TASK-S06/S07/S08）節中與節氣推薦頁、節氣×菜價整合相關的敘述，皆為原始規劃，不代表目前實際行為

另外，**「任務中心儀表板」（原規格 4.2、8、9、10、14.5、15.3、16.3、19.2、20.2 節，T01-T11）從未實作，且確認不影響現有功能，故不再視為 MVP 範圍**：

- 現況：`data/tasks/tasks.json` 不存在，前後端皆無任何任務看板頁面或 API；`docs/dev_logs`、`docs/tutorials`、`docs/handoff` 目錄仍依循本文件第 10 節的文件模板留存個別任務記錄（如 TASK-D07、TASK-D08），但這只是文件撰寫慣例，並非本節所指的任務中心系統
- 本文件第 4.2、8、9、10、11（部分）、13、14.5、15.3、16.3、19.2、20.2、23 節中與任務中心儀表板相關的敘述，皆為原始規劃，不代表目前實際行為，未來也不再規劃補做
