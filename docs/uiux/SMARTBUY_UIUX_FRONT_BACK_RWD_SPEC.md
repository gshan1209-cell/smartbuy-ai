...
後台採儀表板設計，服務平台管理者，並預留中期農民及後期商家使用情境。後台主要負責資料監控、內容管理、會員管理、AI 預測監控與營運決策。
...
- 前端建立 `ProtectedRoute` 與 `RoleGuard`
- 未具後台權限者進入 `/dashboard/*` 應導向前台或顯示 403
- 若後端尚未提供 role，先以 feature flag / development mock 支援，但必須集中管理
- 禁止在各頁散落硬編碼角色判斷

### 9.3 後續後端需求

- `members.role`
- JWT payload 加入 role 或登入後由 `/api/auth/me` 取得
- admin audit log
- 後台 CRUD API 權限驗證
